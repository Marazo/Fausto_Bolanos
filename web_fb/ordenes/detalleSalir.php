...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $(".aceptar").click(function(){
      if (confirm("Decea aceptar la salidad de los materiales") == true) {
        ...
        var items= $( ":checkbox" ).map(function() {return this.id;}).get();
        $.ajax({
          url: '../funciones/aceptarSalida.php',
          type: 'POST',
          data: { hoja: hojaControl, item: items},
  ...
  <body>
    <div class="w3-container w3-wide">
        ...
        <table class="w3-table w3-bordered w3-animate-top w3-striped">
          ...
          if ($result = mysqli_query($mysqli, $consulta->sqlDespachoID($hoja))) {
            while ($filas = mysqli_fetch_array($result)) {
              echo "
              <tr>
                <th><input type='checkbox' id='".$filas['ID_ITEM']."' name='item[]' checked></th>
      ...
      <button type="button" name="button" class="w3-button w3-signal-black w3-bottom aceptar" >Aceptar Salida</button>
